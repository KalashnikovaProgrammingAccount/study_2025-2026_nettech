...
Диаметр домена коллизий — это наибольшее расстояние между любыми двумя узлами этого домена. В нашей топологии используются два повторителя класса 2, а все узлы подключены только витой парой, поэтому согласно таблице ниже предельно допустимый диаметр составляет 205 метров (рис. [-@fig:002]).

![Максимальный диаметр](image/2.png){#fig:002 width=70%}

Составим таблицу: семь строк (первая — номер сегмента, остальные — варианты задания) и восемь столбцов (первый — номер варианта, последний — диаметр, промежуточные — длины сегментов). Сегменты, попадающие в диаметр, выделим (рис. [-@fig:003]).
